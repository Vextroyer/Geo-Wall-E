...
static class OperationTable
{
    ...
    static OperationTable()
    {
        ...
        Register("==", (Element.Number a, Element.Number b) =>
        {
            if (Utils.Compare(a.Value, b.Value) == 0) return Element.TRUE;
            return Element.FALSE;
        });
        Register("==", (Element.Point a, Element.Point b) =>
        {
            if (Operate("==", a.x, b.x) == Element.TRUE && Operate("==", a.y, b.y) == Element.TRUE) return Element.TRUE;
            return Element.FALSE;
        }
        );
        #endregion
